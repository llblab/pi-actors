...
- `Async run observability`: Ambient triangles count unfinished async run instances, not internal command-template branches; to test multi-run parallelism, start several async runs concurrently (for example via parallel tool calls) and hold them open long enough to observe simultaneous triangles. Event-driven terminal/outbox watchers should initiate follow-up only for unhandled exceptional states or explicit script-authored `followup`; do not restore busy-polling loops or duplicate follow-ups for `done`, `failed`, or `cancelled` states already handled by synchronous tool results. | Trigger: Changing async run UI, notifications, event delivery, or smoke-test interpretation | Action: Preserve one-triangle-per-live-run semantics and document any new status dimension separately from internal branch counts.
