...
- `/lib/*.ts`: Flat Domain DAG modules for cohesive reusable behavior; `templates.ts` owns command-template parsing and invocation assembly, `args.ts`/`identity.ts` own names and defaults, `config.ts` owns config persistence, `registry.ts` owns registry register/update/delete use-cases, `output.ts` owns result formatting/truncation, `execution.ts` owns registered-tool execution, `prompts.ts` owns LLM-facing copy, `tools.ts` owns pi-facing tool definitions for both `register_tool` and generated runtime tools, `runtime.ts` owns load/conflict/registration coordination, and `paths.ts` owns config path resolution
- `index.ts` should import local domains as namespaces (`import * as Args from "./lib/args.ts"`) so orchestration reads through domain names instead of flat helper imports
...
- `Current runtime contract`: v0.2.0 registers trusted command templates with named args, optional defaults, split-first argv construction, and `{file}` as the canonical local file path arg | Trigger: Changing registration or invocation behavior | Action: Keep README, command-template docs, implementation, and migration notes aligned
