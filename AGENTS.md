...
- `Current runtime contract`: v0.6.0 registers trusted command templates with tool names from registry keys, placeholder-derived args, progressive typed arg declarations, inline/default config fallback, split-first command-arg construction, sequential or `mode: "parallel"` composition, direct no-shell execution, per-node `delay`, one `template_job` action tool with graceful cancel plus force kill, generic detached template job primitives, coordinator-scoped runner-reported active sub-agent observability and terminal follow-up context, `template` job recipe references, co-located job recipe entries, `~/.pi/agent/jobs/*.json` template job files, and `{file}` as the canonical local file path arg | Trigger: Changing registration or invocation behavior | Action: Keep README, command-template docs, template-job docs, implementation, and migration notes aligned
