...
- Add typed command-template argument declarations.
  - Priority: High.
  - Scope: Keep current string-only `args` fully compatible while progressively accepting compact typed forms such as `file:path`, `out_dir:path`, `timeout:int=60000`, `dry_run:bool=true`, and `mode:enum(check,fix)=check`.
  - Exit: Parser, registry normalization, generated tool schemas, runtime validation/normalization, and docs all support the compact mini-schema without introducing JSON Schema as the authoring format; untyped current `args` declarations continue to work unchanged.
