...
- `[Scripts]` Migrated `recipe-utils`, `build-dist`, `locker`, and `coordinator` command logic behind compiled TypeScript modules while preserving the stable `scripts/*.mjs` shim paths; script-owned TypeScript can now compile from `scripts/*.ts` when the logic is entrypoint-specific rather than reusable library behavior, and project guidance now keeps such logic in `scripts` until a real second consumer justifies promotion to `lib`. The `validate-recipe` implementation now follows that script-owned layout under `scripts/validate-recipe.ts`.
