...
- `[Scripts]` Migrated `recipe-utils`, `build-dist`, `locker`, and `coordinator` command logic behind compiled TypeScript modules while preserving the stable `scripts/*.mjs` shim paths; script-owned TypeScript can now compile from `scripts/*.ts` when the logic is entrypoint-specific rather than reusable library behavior.
