...
export function normalizeControlAction(value) {
    ...
    if (action.length > Limits.CONTROL_ACTION_MAX_LENGTH) {
        throw new Error(`control.action exceeds ${Limits.CONTROL_ACTION_MAX_LENGTH} ASCII characters`);
    }
    return action;
}
export function normalizeControlInput(input) {
    ...
    if (bytes > Limits.CONTROL_INPUT_MAX_BYTES) {
        throw new Error(`control.input exceeds ${Limits.CONTROL_INPUT_MAX_BYTES} serialized bytes`);
    }
...
function normalizeTarget(value) {
    ...
    if (!target.startsWith("run:")) {
        throw new Error(`unsupported control target: ${target}; use run:<id> or runtime`);
    }
...
export function normalizeControlRequest(input) {
    ...
    if (removed.length > 0) {
        throw new Error(`actor-message fields are removed: ${removed.sort().join(", ")}; use target, action, input, verbose`);
    }
