...
export function normalizeDraftConsolidationInventory(records) {
    return records
        .flatMap((record) => {
        ...
        return [{
                ...
                ...(Array.isArray(record.risk_labels)
                    ? {
                        riskLabels: record.risk_labels.filter((label) => typeof label === "string"),
                    }
                    : {}),
...
export function validateDraftConsolidationPlan(inventory, plan) {
    const errors = [];
    ...
    const targetPreconditions = new Map();
    ...
    for (const item of plan.drafts) {
        ...
        if (item.action === "discard") {
        ...
        else if (item.targetSha256 !== null &&
            ...
            if (targetPreconditions.has(item.target) &&
            ...
            if ((item.action === "promote" && priorAction !== undefined) ||
    ...
    for (const item of inventory) {
        if (!seen.has(item.path))
            errors.push(`missing draft decision: ${item.path}`);
    }
