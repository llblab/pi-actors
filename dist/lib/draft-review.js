...
export function findUnsafeRecipeReason(value, key = "") {
    if (key && SECRET_KEY.test(normalizedKey(key)))
        return `secret-bearing key: ${key}`;
    if (typeof value === "string") {
        if (PRIVATE_KEY_TEXT.test(value))
            return "private-key material";
        if (CREDENTIAL_TOKEN.test(value))
            return "credential-shaped value";
        if (AWS_SECRET_REFERENCE.test(value))
            return "AWS credential reference";
        if (SECRET_MANAGER_REFERENCE.test(value))
            return "secret-manager reference";
        if (SECRET_TEXT.test(value))
            return "secret-bearing value";
        if (TEMPORARY_PATH.test(value))
...
function validateAssessment(value, draft, errors) {
    ...
    if (!finiteScore(value.flexibility) ||
...
export function validateDraftReviewInput(input) {
    const errors = [];
    if (!UUID_PATTERN.test(input.batchId))
        errors.push("review input batchId must be a UUID");
...
export function validateDraftReviewResult(input, result) {
    const errors = [...validateDraftReviewInput(input).errors];
    if (result.batchId !== input.batchId)
        errors.push("review result batch identity mismatch");
    ...
    for (const decision of result.decisions) {
        ...
        if (validateAssessment(decision.assessment, draftPath, errors)) {
            const recordedLaunches = Number(source.usage?.lifetime_calls ?? source.usage?.calls ?? 0);
        ...
        if (typeof source.usage?.demoted_fingerprint === "string" &&
            source.usage.demoted_fingerprint === source.usage.fingerprint) {
            errors.push(`demoted draft requires a revision before automatic promotion: ${draftPath}`);
        }
        ...
        if (!isRecord(source.recipe) || !Object.hasOwn(source.recipe, "template")) {
            ...
            if (unsafe)
                errors.push(`unsafe promotion source (${unsafe}): ${draftPath}`);
        }
    }
    for (const draft of input.drafts) {
        if (!seen.has(draft.path))
            errors.push(`missing review decision: ${draft.path}`);
    }
    return { errors, ok: errors.length === 0 };
}
export function parseDraftReviewResult(stdout) {
    const marker = "DRAFT_REVIEW_RESULT";
    const markerIndex = stdout.lastIndexOf(marker);
    if (markerIndex < 0)
        throw new Error("Draft reviewer output marker is missing.");
    ...
    if (!isRecord(value))
        throw new Error("Draft reviewer result must be an object.");
    return value;
}
export function createDraftReviewPrompt(inputPath) {
    return [
