...
function statePath(scope, options) {
    return scope === "draft"
        ? options.draftStatePath ?? Paths.getDraftSleepStatePath()
        : options.toolStatePath ?? Paths.getToolReviewStatePath();
}
function schedule(scope, options) {
    if (scope === "draft")
        options.scheduleDraft?.();
    else
        options.scheduleTool?.();
}
export function controlAutomaticReview(action, scope, options = {}) {
    ...
    const result = withFileMutationLock(path, () => {
        ...
        const resumeDraftTransaction = scope === "draft" &&
