...
function readJson(path) {
    if (!path || !existsSync(path) || statSync(path).size > MAX_FILE_BYTES)
        return undefined;
    try {
...
function strings(value, limit = 20) {
    return Array.isArray(value)
        ? value.filter((entry) => typeof entry === "string").slice(-limit)
        : [];
}
...
function draftEvidence(state) {
    ...
    const decisions = Array.isArray(evidence.decisions)
        ? evidence.decisions
        : [];
    const garbage = evidence.garbageCollection;
    return {
        actions: Object.fromEntries(["promote", "discard"].map((action) => [
            action,
...
function toolEvidence(state) {
    ...
    const decisions = Array.isArray(approved.decisions)
        ...
        : [];
    return {
        actions: Object.fromEntries(["keep", "evolve", "replace", "demote", "merge", "split"].map((action) => [
            action,
            decisions.filter((decision) => decision.action === action).length,
        ])),
        ...
        transaction_phase: readJson(join(dirname(text(state?.approvedPath, 4096)), "journal.json"))?.phase,
    };
}
function lineageSummaries(recipeRoot) {
...
export function readAutomaticReviewDiagnostics(options = {}) {
