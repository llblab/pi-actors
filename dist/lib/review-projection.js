 ...
 */
function occurrenceId(prefix, index) {
    return `${prefix}_${String(index + 1).padStart(2, "0")}`;
}
export const draftReviewIdentity = (index) => occurrenceId("draft", index);
export const toolReviewIdentity = (index) => occurrenceId("tool", index);
function contentGroups(hashes) {
...
function templateShape(value) {
    if (typeof value === "string")
        return { kind: "command", nodes: 1 };
    ...
    return {
        ...
        nodes: 1 + children.reduce((total, child) => total + Number(templateShape(child).nodes ?? 0), 0),
    };
}
function contractShape(recipe) {
    if (!recipe)
        return undefined;
    return {
...
export function projectDraftReviewInput(input) {
    return {
...
export function restoreDraftReviewResult(input, result) {
...
export function projectToolReviewInput(input) {
    return {
        createdAt: input.createdAt,
...
export function restoreToolReviewResult(input, result) {
