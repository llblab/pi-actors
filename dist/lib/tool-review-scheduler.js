...
import * as ToolReviewLineage from "./tool-review-lineage.js";
import * as ToolReviewLineageTransaction from "./tool-review-lineage-transaction.js";
import * as ToolReviewTransaction from "./tool-review-transaction.js";
import { findUnsafeRecipeReason } from "./draft-review.js";
import { parseToolReviewResult, TOOL_REVIEW_THRESHOLD, validateToolReviewInput, validateToolReviewResult, } from "./tool-review.js";
export const TOOL_REVIEW_RESULT_MAX_BYTES = 16 * 1024 * 1024;
function sha256(path) {
    return createHash("sha256").update(readFileSync(path)).digest("hex");
}
function readAdmissionState(path) {
    try {
...
function usageOrder(tool) {
    const firstSeen = tool.usage?.first_seen;
    return typeof firstSeen === "string" ? firstSeen : "";
}
export function listEligibleToolReviewRecipes(recipeRoot = Paths.getRecipeRoot(), now = new Date()) {
    ...
    return [...discovered.active.values()]
        ...
        .flatMap((entry) => {
        if (!RecipesUsage.ensureRecipeLineage(entry.path, now, recipeRoot))
            return [];
        if (RecipesUsage.isCurrentRecipeRevisionReviewed(entry.path, recipeRoot))
            return [];
        ...
        .sort((left, right) => usageOrder(left).localeCompare(usageOrder(right)) ||
        left.name.localeCompare(right.name));
}
function captureToolReviewBatchLocked(options, recipeRoot, statePath, now) {
    ...
    if (eligible.length < TOOL_REVIEW_THRESHOLD)
        return undefined;
    const reviewId = (options.createReviewId ?? randomUUID)();
    const inputPath = join((options.batchRoot ?? Paths.getToolReviewBatchDir)(reviewId), "input.json");
...
export function captureToolReviewBatch(options = {}) {
    ...
    return withFileMutationLock(statePath, () => captureToolReviewBatchLocked(options, recipeRoot, statePath, now));
}
function approvedTargets(input, decisions, recipeRoot) {
    const targets = new Map();
    const tools = new Map(input.tools.map((tool) => [tool.name, tool]));
    const add = (name, path, recipe, lineage, source) => {
    ...
    for (const decision of decisions) {
        ...
        if (decision.action === "demote") {
            add(decision.source, join(recipeRoot, "drafts", `${decision.source}.json`), source.recipe, "demote", decision.source);
            continue;
        }
        if (decision.action === "evolve") {
            const source = tools.get(decision.source);
            add(decision.target, join(recipeRoot, `${decision.target}.json`), source.recipe, "evolve", decision.source);
            continue;
        }
        if (decision.action === "merge") {
            const source = tools.get(decision.source);
            add(decision.target, join(recipeRoot, `${decision.target}.json`), source.recipe, "merge", decision.source);
            continue;
        }
        throw new Error(`${decision.action} requires explicit operator-authored recipe mutation.`);
    }
    return [...targets.values()].sort((left, right) => left.path.localeCompare(right.path));
}
export function processToolReviewResult(state, deps) {
    if (!state.runId) {
    ...
    try {
        ...
        result = ReviewProjection.restoreToolReviewResult(input, parseToolReviewResult(readFileSync(stdoutPath, "utf8")));
    }
    catch (error) {
        return {
            error: error instanceof Error ? error.message : String(error),
...
export function applyApprovedToolReviewAtSessionBoundary(options = {}) {
    const statePath = options.statePath ?? Paths.getToolReviewStatePath();
    const recipeRoot = options.recipeRoot ?? Paths.getRecipeRoot();
    return withFileMutationLock(statePath, () => {
        ...
        try {
            ...
            let lineage;
            try {
                lineage = ToolReviewLineageTransaction.finalizeToolReviewLineage(state.approvedPath, {
                    now: options.now,
                    recipeRoot,
                });
            }
...
export function createToolReviewScheduler(deps) {
        ...
        timer = setTimeout(() => {
            ...
            try {
                withFileMutationLock(statePath, () => {
                    ...
                    let batch;
                    try {
                        batch = captureToolReviewBatchLocked(deps, recipeRoot, statePath, now);
                        if (!batch)
                            return;
...
export function toolReviewRunId(reviewId) {
