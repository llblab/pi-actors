...
function latestControlTimestamp(control) {
    return (control.failed_at ??
        control.handled_at ??
        control.claimed_at ??
        control.delivered_at ??
        control.queued_at);
}
function diagnosticItems(diagnostics) {
...
function controlDiagnosticItems(diagnostics) {
    return diagnostics.slice(-SOURCE_LIMIT).map((diagnostic, index) => ({
...
function controlItems(stateDir) {
    ...
    const projected = read.records.slice(start).map((control, index) => {
        const value = ControlProjection.projectRunControl(control);
        return ordered({
            id: `control:${value.id}`,
    ...
    return [
        ...projected,
        ...controlDiagnosticItems(read.diagnostics).map((item, index) => ordered(item, "control", read.records.length + index + 1)),
    ];
}
function agentItems(stateDir) {
...
function resultItems(stateDir) {
    ...
    const read = readJsonFileResilient(path, {});
    if (Object.keys(read.value).length === 0) {
        return diagnosticItems(read.diagnostics).map((item, index) => ordered(item, "process:result", index));
    }
    const stat = statSync(path);
...
function artifactItems(stateDir, declarations) {
    ...
    if (!manifest)
        return [];
    return Object.entries(manifest).slice(0, SOURCE_LIMIT).map(([name, artifact], index) => ordered({
        id: `artifact:${name}`,
        ...
        ...(artifact.required && !artifact.exists ? { level: "warning" } : {}),
        detail: redact({
