...
- `inspect`: intentionally read owned `run:<id>` status, tail, events, artifacts, files, or mailbox metadata; read `coordinator`, `session:<id>`, or `session:all` run inventory with optional status filtering; read `tool:<name>` status or schema for registered tool actors.
