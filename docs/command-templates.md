...
Layer boundary: command templates own only the synchronous execution graph. Recipe imports, import-reference expressions, recipe lookup, `async: true`, run ids, state dirs, mailbox controls, and actor-message delivery are host/recipe/async-run configuration layers, not portable command-template syntax.
...
- Detached lifecycle, run ids, state dirs, logs, cancellation, mailbox controls, or actor-message delivery.
