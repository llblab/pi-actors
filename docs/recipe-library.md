...
- `recipes/utility-actor-message.json`: Deterministically wrap stdin as an addressed actor-message envelope with the same public names as the envelope: `to`, `from`, `type`, `delivery`, `summary`, `body`, optional `correlation_id`/`reply_to`, and `metadata`.
