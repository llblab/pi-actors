...
export function resolveRecipePath(
  ...
  return resolve(recipeRoot, trimmed.endsWith(".json") ? trimmed : `${trimmed}.json`);
}
...
function readRawRecipeConfig(path: string): Record<string, unknown> | undefined {
  if (!existsSync(path)) return undefined;
  try {
    const raw = JSON.parse(readFileSync(path, "utf8")) as Record<string, unknown>;
    return raw && typeof raw === "object" && !Object.hasOwn(raw, "tool") ? raw : undefined;
  } catch {
    return undefined;
  }
}
...
function getRecipeImports(raw: Record<string, unknown>): Record<string, TemplateRecipeImport> {
  ...
  for (const [alias, value] of Object.entries(raw.imports)) {
    if (!/^[A-Za-z0-9_.-]+$/.test(alias)) throw new Error(`Invalid recipe import alias: ${alias}`);
    if (typeof value === "string") {
      result[alias] = value;
      continue;
    }
    if (!isRecord(value)) throw new Error(`Recipe import must be a string or object: ${alias}`);
...
function resolveImportRef(
  ...
  for (const [alias, imported] of Object.entries(imports)) {
    ...
    if (resolved === undefined && !allowMissing) throw new Error(`Unknown recipe import reference: ${ref}`);
...
function parseImportLiteral(value: string, imports: Record<string, ImportedRecipe>): unknown {
  const trimmed = value.trim();
...
function evaluateImportExpression(content: string, imports: Record<string, ImportedRecipe>): { matched: boolean; value: unknown } {
...
function substituteImportRefs(value: unknown, imports: Record<string, ImportedRecipe>): unknown {
  if (typeof value === "string") {
  ...
  if (Array.isArray(value)) return value.map((item) => substituteImportRefs(item, imports));
  if (isRecord(value)) {
    const result: Record<string, unknown> = {};
    for (const [key, child] of Object.entries(value)) result[key] = substituteImportRefs(child, imports);
    return result;
  }
  return value;
}
  ...
  if (typeof template === "object" && !Array.isArray(template)) {
    return {
      ...
      ...(mergeDefaults(template.defaults, defaults, isRecord(cleanOverrides.defaults) ? cleanOverrides.defaults : undefined)
        ? { defaults: mergeDefaults(template.defaults, defaults, isRecord(cleanOverrides.defaults) ? cleanOverrides.defaults : undefined) }
        : {}),
...
function expandImportNodes(
  ...
  if (Array.isArray(value)) {
    return value.map((item) => expandImportNodes(item, imports) as CommandTemplateConfig);
  }
  ...
  if (importAlias) {
    ...
    const nodeDefaults = isRecord(record.defaults) ? record.defaults : undefined;
  ...
  if (Array.isArray(record.template)) {
    return {
      ...record,
      template: record.template.map((item) => expandImportNodes(item as CommandTemplateValue, imports) as CommandTemplateConfig),
    } as CommandTemplates.CommandTemplateObjectConfig;
  }
  if (record.template && typeof record.template === "object") {
    return {
      ...record,
      template: expandImportNodes(record.template as CommandTemplateValue, imports),
    } as CommandTemplates.CommandTemplateObjectConfig;
  }
  return value;
}
...
export function readResolvedRecipeConfig(
  ...
  const substituted = substituteImportRefs(raw, imports) as Record<string, unknown>;
  ...
  return {
    ...
    ...(substituted.async === true ? { async: true } : substituted.async === false ? { async: false } : {}),
    ...(typeof substituted.state_dir === "string" ? { state_dir: substituted.state_dir } : {}),
    ...(Object.keys(imports).length > 0 ? { imports: getRecipeImports(raw) } : {}),
    template: expandedTemplate,
    ...(Array.isArray(substituted.args) ? { args: substituted.args as string[] } : {}),
    ...(isRecord(substituted.defaults) ? { defaults: substituted.defaults } : {}),
    ...(substituted.mode === "parallel" || substituted.mode === "sequence" ? { mode: substituted.mode } : {}),
    ...(typeof substituted.label === "string" ? { label: substituted.label } : {}),
    ...(typeof substituted.timeout === "number" ? { timeout: substituted.timeout } : {}),
    ...(typeof substituted.delay === "number" ? { delay: substituted.delay } : {}),
    ...(typeof substituted.output === "string" ? { output: substituted.output } : {}),
    ...(typeof substituted.retry === "number" ? { retry: substituted.retry } : {}),
    ...(typeof substituted.critical === "boolean" ? { critical: substituted.critical } : {}),
    ...(substituted.failure === "continue" || substituted.failure === "branch" || substituted.failure === "root" ? { failure: substituted.failure } : {}),
    ...(substituted.recover !== undefined ? { recover: substituted.recover as CommandTemplateValue } : {}),
    ...(typeof substituted.repeat === "number" ? { repeat: substituted.repeat } : {}),
    ...(isRecord(substituted.values) ? { values: substituted.values } : {}),
  };
}

export function getRecipeTemplate(value: unknown): CommandTemplateValue | undefined {
