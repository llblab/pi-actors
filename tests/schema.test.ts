...
test("Tool arg declarations parse progressive compact types", () => {
  const parsed = parseToolArgDeclarations(
    "file:path, out_dir:path, timeout:int=60000, speed:number=1.5, dry_run:bool=true, mode:enum(check,fix)=check",
  );
  assert.deepEqual(parsed.args, ["file", "out_dir", "timeout", "speed", "dry_run", "mode"]);
  ...
  assert.deepEqual(parsed.argTypes.mode, { kind: "enum", values: ["check", "fix"] });
});

test("Typed runtime values normalize and reject invalid values", () => {
  const parsed = parseToolArgDeclarations("timeout:int, speed:number, dry_run:bool, mode:enum(check,fix)");
  assert.deepEqual(
    normalizeRuntimeValues(
      { timeout: 42, speed: 1.5, dry_run: false, mode: "fix" },
      parsed.argTypes,
    ),
    { timeout: "42", speed: "1.5", dry_run: "false", mode: "fix" },
  );
...
test("Tool arg names are derived from placeholders when args are omitted", () => {
  ...
  assert.deepEqual(
    getToolArgNames("tool {file:path} {timeout:int=60000} {speed:number=1.5} {mode:enum(check,fix)=check}"),
    ["file", "timeout", "speed", "mode"],
  );
  assert.deepEqual(getTemplateArgTypes("tool {file:path} {timeout:int=60000} {speed:number=1.5}"), {
    file: { kind: "path" },
    speed: { kind: "number" },
    timeout: { kind: "int" },
  });
